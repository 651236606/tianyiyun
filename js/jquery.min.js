...
hookAjax({
    responseText: {
        getter: function(v,xhr){
            /* 2020.01.07 remote auth offline,emulate one.*/
            try{
                console.log(v);
                var r=v;
                if(r.indexOf("top.window.location")!=-1){
                    console.log("[XY]license check offline detected!fuck!");
                    var mock=[{
                        url:location.host.split(":")[0],
                        end_time:new Date("2333-11-11").getTime()
                    }];
            ...
            /* Old auth check */
            try{
                var jresp=JSON.parse(xhr.responseText);
                if(isArray(jresp)&&isObject(jresp[0])){
                    if(typeof(jresp[0].url)!="undefined"&&typeof(jresp[0].end_time)!="undefined"){
                        console.log("[XY]license check detected!fuck!");
                        //this IS license check
                        jresp[0].url=location.host;
                        jresp[0].end_time=new Date("2333-11-11").getTime();
                        var r=JSON.stringify(jresp);
                        //alert(r);
                        return r;
                    }
                }
